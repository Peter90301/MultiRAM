...
from config_unified import CfgBioPIM, CfgDRAM3D, CfgPIM
    ...
    for profile, query_fastq, buckets in groups:
        ...
        for label in labels:
            cmd = [
                ...
                "--gpu-tdp-w",
                str(args.gpu_tdp_w),
                "--cudapoa-cmd-template",
                args.cudapoa_cmd_template,
                "--gpu-pcie-gbps",
...
def _select_query_sequences(
    query_fasta: Optional[str],
    query_fastq: Optional[str],
    max_queries: int,
    read_length_target: int,
    target_total_bases: int,
) -> List[str]:
    if query_fastq and os.path.exists(query_fastq):
        seqs = parse_fastq(query_fastq)
    elif query_fasta and os.path.exists(query_fasta):
        seqs = parse_fasta(query_fasta)
    else:
        return []

    if read_length_target > 0:
        target = int(read_length_target)
        filtered: List[str] = []
        for s in seqs:
            if len(s) < target:
                continue
            filtered.append(s[:target])
        seqs = filtered

    if max_queries > 0 and len(seqs) > max_queries:
        seqs = seqs[:max_queries]

    if target_total_bases > 0 and seqs:
        trimmed: List[str] = []
        total = 0
        for s in seqs:
            if total >= target_total_bases:
                break
            if total + len(s) <= target_total_bases:
                trimmed.append(s)
                total += len(s)
                continue
            keep = target_total_bases - total
            if keep > 0:
                trimmed.append(s[:keep])
                total += keep
            break
        seqs = [s for s in trimmed if s]
...
def _run_gpu_benchmark(
    ...
    if not query_sequences:
        raise RuntimeError("No query sequences available for cudapoa benchmark.")
    ...
    def _parse_nsys_kernel_time_s(report_text: str) -> Optional[float]:
        # Parse the nsys "cuda_gpu_kern_sum" stats table.
        # Example block:
        # [6/8] Executing 'cuda_gpu_kern_sum' stats report
        # Time (%)  Total Time (ns)  Instances  Avg (ns) ... Name
        #  76.7            25888          1      25888 ... kernel_name
        total_ns = 0
        in_cuda_kern_sum = False
        saw_header = False

        for raw_line in report_text.splitlines():
            line = raw_line.strip()
            if not line:
                # keep scanning; nsys blocks may contain empty lines
                continue

            if "cuda_gpu_kern_sum" in line:
                in_cuda_kern_sum = True
                saw_header = False
                continue

            if not in_cuda_kern_sum:
                continue

            if ("Time (%)" in line) and ("Total Time" in line) and ("Instances" in line):
                saw_header = True
                continue
            ...
            # Data row: first col=percent, second col=Total Time(ns), third col=Instances
            # Handles numbers with and without commas.
            m = re.match(r"^([0-9]+(?:\.[0-9]+)?)\s+([0-9,]+)\s+([0-9,]+)\s+", line)
            if m:
                try:
            ...
            # End of cuda_gpu_kern_sum block when next report section starts.
            if line.startswith("[") and "Executing" in line:
                break
            if line.startswith("Generated:"):
                break

        return (total_ns * 1e-9) if total_ns > 0 else None

    wall_t0 = time.time()
    with tempfile.TemporaryDirectory(prefix="cudapoa_bench_") as td:
        input_fa = os.path.join(td, "queries.fa")
        output_fa = os.path.join(td, "consensus.fa")
        _write_fasta(input_fa, query_sequences)

        for rep in range(max(1, repeats)):
            cmd_str = cudapoa_cmd_template.format(
            ...
            if not cmd:
                raise RuntimeError("Empty cudapoa command after formatting --cudapoa-cmd-template")
            ...
            try:
                ...
                raise RuntimeError(
                    "Cannot execute cudapoa due to permission/noexec policy. "
                    "Set --cudapoa-cmd-template to an executable path under an exec-mounted directory."
                ) from e
            rep_wall_s = (time.time() - t0)
            elapsed_total += rep_wall_s

            # nsys typically prints stats to stderr; parse both streams to recover pure kernel time.
            nsys_text = (run.stdout or "") + "\n" + (run.stderr or "")
            rep_elapsed_s = _parse_nsys_kernel_time_s(nsys_text)
            if rep_elapsed_s is not None and rep_elapsed_s > 0:
                elapsed_s += rep_elapsed_s
                print(
                    "GPU_BENCH_DEBUG "
                    f"rep={rep} parsed_kernel_s={rep_elapsed_s:.6f} "
                    f"rep_wall_s={rep_wall_s:.6f} source=nsys"
                )
            else:
                # Fallback to wall-clock for robustness when nsys stats are unavailable.
                elapsed_s += rep_wall_s
                print(
                    "GPU_BENCH_DEBUG "
                    f"rep={rep} parsed_kernel_s=NA "
                    f"rep_wall_s={rep_wall_s:.6f} source=wall_fallback"
                )
    ...
    if sampled_avg_power_w is None or sampled_avg_power_w < 60.0:
    ...
    print(
        "GPU_BENCH_DEBUG "
        f"parsed_kernel_total_s={elapsed_s:.6f} "
        f"wall_total_s={wall_s:.6f} "
        f"elapsed_total_s={elapsed_total:.6f}"
    )

    # Energy should be based on parsed pure kernel time, not system wall-clock overhead.
    energy_j = avg_power_w * elapsed_s
    total_queries = float(len(query_sequences) * max(1, repeats))

    return {
        # Return pure kernel-time metric parsed from nsys stats.
        "elapsed_s": elapsed_s,
        "wall_s": wall_s,
...
def main() -> None:
    ...
    parser.add_argument(
        "--pim-aligner",
        type=str,
        choices=["gwfa", "minigraph_like"],
        default="gwfa",
        help="Alignment backend used to derive PIM workload",
    )
    ...
    parser.add_argument(
        "--cudapoa-cmd-template",
        type=str,
        default="nsys profile --stats=true cudapoa -i {input_fasta}",
        help="GPU S2G command template. Placeholders: {input_fasta}, {output_fasta}, {repeat}",
    )
    ...
    bio = run_seq_to_graph_alignment(
        ...
        aligner_backend=args.pim_aligner,
    )
    ...
    print(f"  pim_aligner={args.pim_aligner}")
    ...
    # Ensure align ops scale with read length even if upstream alignment summary degrades.
    ops_per_align_task = int(bio["ops_per_align_task"])
    read_len_ref_bp = 200.0
    scaled_floor_ops = int(CfgBioPIM.ops_per_align_task * max(1.0, float(avg_len) / read_len_ref_bp))
    if ops_per_align_task < scaled_floor_ops:
        print(
            "[WARN] ops_per_align_task appears under-scaled for read length; "
            f"raise from {ops_per_align_task} to {scaled_floor_ops} (avg_query_len={avg_len})"
        )
        ops_per_align_task = scaled_floor_ops
    ...
    pim = _run_pim(
        ...
        ops_per_align_task=ops_per_align_task,
    ...
    print("\n=== Running GPU benchmark (cudapoa S2G) ===")
    query_sequences = _select_query_sequences(
        query_fasta=args.query_fasta,
        query_fastq=args.query_fastq,
        max_queries=args.max_queries,
        read_length_target=args.read_length_target,
    ...
    if not query_sequences:
        raise RuntimeError(
            "No query sequences available for GPU cudapoa benchmark. "
            "Check --query-fastq/--query-fasta and read-length filters."
        )
    if len(query_sequences) != int(bio["num_queries"]):
        print(
            "[WARN] GPU query count differs from derived workload: "
            f"gpu={len(query_sequences)}, derived={bio['num_queries']}"
        )

    gpu = _run_gpu_benchmark(
        cudapoa_cmd_template=args.cudapoa_cmd_template,
